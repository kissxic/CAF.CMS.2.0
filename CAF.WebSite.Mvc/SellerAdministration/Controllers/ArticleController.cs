...
{
    public class ArticleController : SellerAdminControllerBase
    {
        #region Fields
        ...
        protected void UpdateArticleGeneralInfo(Article article, ArticleModel model)
        {
            ...
            a.StatusId = (int)ArticleStatus.Audit;
            ...
            a.VendorId = _workContext.CurrentVendor.Id;
        ...
        protected void PrepareArticleModel(ArticleModel model, Article article, bool setPredefinedValues, bool excludeProperties)
        {
            ...
            if (article != null)
            {
                ...
                var parentCategory = _categoryService.GetArticleCategoryById(model.CategoryId.Value);
        ...
        public ActionResult Create(PublicStepOneModel pmodel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageArticles))
                return AccessDeniedView();
            if (!_channelService.IsPublicArticle(pmodel.ChannelId ?? 0, _workContext.CurrentUser.Id))
                return RedirectToAction("Index", "Recharge", new { area = "Seller", channelId = pmodel.ChannelId });
            ...
            PrepareArticleModel(model, null, true, false);
        ...
        public ActionResult Create(string btnId, string formId, ArticleModel model, bool continueEditing, bool continueCreate, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                return continueEditing ? RedirectToAction("Edit", new { id = article.Id }) :
                   RedirectToAction("List", new { SearchChannelId = model.ChannelId });
        ...
        public ActionResult Edit(string btnId, string formId, ArticleModel model, bool continueEditing, bool continueCreate, FormCollection form)
        {
            ...
            if (article == null || article.Deleted)
            {
                return RedirectToAction("List", new { channelId = model.ChannelId });
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (continueCreate)
                {
                    return RedirectToAction("Create", new { btnId = btnId, formId = formId, channelId = model.ChannelId });
                }
                return continueEditing ? RedirectToAction("Edit", new { id = article.Id }) :
                 RedirectToAction("List", new { SearchChannelId = model.ChannelId });
        ...
        public ActionResult Delete(int id)
        {
            ...
            return RedirectToAction("List", new { SearchChannelId = article.ChannelId });
