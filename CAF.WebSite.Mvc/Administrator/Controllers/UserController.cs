...
namespace CAF.WebSite.Mvc.Admin.Controllers
{
    ...
    public partial class UserController : AdminControllerBase
    {
        ...
        [NonAction]
        protected virtual void PrepareMemderGradesModel(UserModel model)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            model.AvailableMemberGrades.Add(new SelectListItem
            {
                Text = _localizationService.GetResource("Admin.Users.Users.Fields.MemberGrade.None", defaultValue: "请选择"),
                Value = "0"
            });
            var memberGrades = this._memberGradeService.GetAllMemberGrades();
            foreach (var memberGrade in memberGrades)
            {
                model.AvailableMemberGrades.Add(new SelectListItem
                {
                    Text = memberGrade.GradeName,
                    Value = memberGrade.Id.ToString()
                });
            }
        }
        [NonAction]
        ...
        public ActionResult UserList(DataTablesParam dataTableParam, UserListModel model, [ModelBinderAttribute(typeof(CommaSeparatedModelBinder))] int[] searchUserRoleIds)
        {
            ...
            var users = _userService.GetAllUsers(null, null,
                ...
                false, null, dataTableParam.PageIndex, dataTableParam.PageSize);
        ...
        public ActionResult Create()
        {
            ...
            PrepareMemderGradesModel(model);
            //default value
            model.Active = true;
        ...
        public ActionResult Create(UserModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_userSettings.CountryEnabled && _userSettings.StateProvinceEnabled && _userSettings.CityEnabled)
                    _genericAttributeService.SaveAttribute(user, SystemUserAttributeNames.CityId, model.StateProvinceId);
                if (_userSettings.CountryEnabled && _userSettings.StateProvinceEnabled)
                    _genericAttributeService.SaveAttribute(user, SystemUserAttributeNames.StateProvinceId, model.StateProvinceId);
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.MemberGradeId = user.MemberGradeId;
        ...
        #region ChannelPublicNum
        public ActionResult UserArticlePublicNumList(int userId, DataTablesParam dataTableParam)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageUsers))
                return AccessDeniedView();
            ...
            var channelPublic = _channelService.GetUserArticlePublicNumsByUserId(userId);
            var total = channelPublic.Count();
            var result = new DataTablesData
            {
                iTotalRecords = total,
                iTotalDisplayRecords = total,
                sEcho = dataTableParam.sEcho,
                aaData = channelPublic.Select(x =>
                {
                    var model = new UserArticlePublicNumModel();
                    model.Id = x.Id;
                    model.PublicedTotal = x.PublicedTotal;
                    model.PublicTotal = x.PublicTotal;
                    model.ChannelName = x.Channel.Title;
                    model.UnLimit = x.UnLimit ? 1 : 0;
                    return model;
                }).Cast<object>().ToArray(),
            };
            return new JsonResult
        ...
        [HttpPost]
        public ActionResult BulkEditPublicNumSave(List<String> updatedPublics)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageUsers))
                return AccessDeniedView();


            if (updatedPublics != null)
            {
                foreach (var pModel in updatedPublics)
                {
                    //update
                    var values = pModel.Split('|');
                    var values2 = values[1].Split(',');
                    var channelPublic = _channelService.GetUserArticlePublicNumsById(values[0].ToInt());
                    if (channelPublic != null)
                    {
                        channelPublic.PublicTotal = values2[1].ToInt();
                        channelPublic.PublicedTotal = values2[2].ToInt();
                        channelPublic.UnLimit = values2[3].ToInt() > 0;
                        _channelService.UpdateUserArticlePublicNum(channelPublic);
                    }
                }
            }
