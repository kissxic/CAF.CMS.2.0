...
namespace CAF.WebSite.Mvc.Admin.Controllers
{
    ...
    public partial class ThemeController : AdminControllerBase
    {
        ...
        /// <returns></returns>
        public ActionResult FilesPreview(FilePreview model)
        {

            if (model.Theme.Contains(".") || model.Theme.Contains("/") || model.Theme.Contains("\\")
                || model.Name.Contains("..") || model.Name.Contains("/") || model.Name.Contains("\\"))
                return null;

            model.Name = model.Name.Replace('|', '\\');

            string filePath;
            if (model.Name.ToLowerInvariant().EndsWith(".cshtml"))
                filePath = Path.Combine(Server.MapPath("~/Themes"), model.Theme, "Views", model.Name);
            else if (model.Name.ToLowerInvariant().EndsWith(".css") || model.Name.ToLowerInvariant().EndsWith(".less"))
                filePath = Path.Combine(Server.MapPath("~/Themes"), model.Theme, "Content", model.Name);
            else return
                    null;


            model.Content = System.IO.File.ReadAllText(filePath);
            model.Name = model.Name.Replace('\\', '|');
            return View(model);
        }
        /// <summary>
        ...
        /// <returns></returns>
        [ValidateInput(false)]
        [HttpPost, ActionName("FilesPreview")]
        public ActionResult SaveFile(FilePreview model)
        {
            if (model.Theme.Contains(".") || model.Theme.Contains("/") || model.Theme.Contains("\\")
                || model.Name.Contains("..") || model.Name.Contains("/") || model.Name.Contains("\\"))
                return null;
            ...
            System.IO.File.WriteAllText(filePath, model.Content, Encoding.UTF8);
            model.Name = model.Name.Replace('\\', '|');
            return View(model);
        }
