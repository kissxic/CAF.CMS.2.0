...
namespace CAF.WebSite.Application.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        public virtual string GetThumbLocalPath(Picture picture, int targetSizeWidth = 0,
            int targetSizeHeight = 0, bool showDefaultPicture = true)
        {
            // 'GetPictureUrl' takes care of creating the thumb when not created already
            string url = this.GetPictureUrl(picture, targetSizeWidth, targetSizeHeight, showDefaultPicture);

            if (url.HasValue())
            {
                var settings = this.CreateResizeSettings(targetSizeWidth, targetSizeHeight);
        ...
        public virtual string GetDefaultPictureUrl(int targetSizeWidth = 0,
            int targetSizeHeight = 0, PictureType defaultPictureType = PictureType.Entity, string storeLocation = null)
        {
            ...
            var url = this.GetProcessedImageUrl(
                ...
                targetSizeWidth, targetSizeHeight,
                storeLocation);

            return url;
        }

        protected internal virtual string GetProcessedImageUrl(object source, int? pictureId, string seoFileName, string extension, int targetSizeWidth = 0,
            int targetSizeHeight = 0, string storeLocation = null)
        {
            var cachedImage = _imageCache.GetCachedImage(
                pictureId,
                seoFileName,
                extension,
                this.CreateResizeSettings(targetSizeWidth, targetSizeHeight));
                    ...
                    if (!File.Exists(cachedImage.LocalPath)) // check again
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                using (var resultStream = _imageResizerService.ResizeImage(sourceStream, targetSizeWidth, targetSizeHeight, _mediaSettings.DefaultImageQuality))
                                {
        ...
        public virtual string GetPictureUrl(
            int pictureId,
            int targetSizeWidth = 0,
            int targetSizeHeight = 0,
            bool showDefaultPicture = true,
            string storeLocation = null,
            PictureType defaultPictureType = PictureType.Entity)
        {
            var picture = GetPictureById(pictureId);
            return GetPictureUrl(picture, targetSizeWidth, targetSizeHeight, showDefaultPicture, storeLocation, defaultPictureType);
        }
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            ...
            if (picture == null || pictureBinary == null || pictureBinary.Length == 0)
            {
                if (showDefaultPicture)
                {
                    url = GetDefaultPictureUrl(targetSizeWidth, targetSizeHeight, defaultPictureType, storeLocation);
                }
                return url;
            }
        ...
        private ResizeSettings CreateResizeSettings(int targetSizeWidth = 0,
            int targetSizeHeight = 0)
        {
            var settings = new ResizeSettings();
            if (targetSizeWidth > 0)
            {
                settings.MaxWidth = targetSizeWidth;
                settings.MaxHeight = targetSizeHeight;
            }
