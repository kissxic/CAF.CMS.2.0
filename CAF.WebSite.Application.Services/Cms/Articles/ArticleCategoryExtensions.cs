    ...
    public static class CategoryExtensions
    {
        ...
        public static IList<int> CategoriesForTree(this IList<ArticleCategory> source, int parentId = 0, bool ignoreCategoriesWithoutExistingParent = false)
        {
            ...
            if (!ignoreCategoriesWithoutExistingParent && result.Count != source.Count)
            {
                // find categories without parent in provided category source and insert them into result
                foreach (var cat in source)
                    if (result.Where(x => x == cat.Id).Count() <= 0)
                        result.Add(cat.Id);
            }
            return result;
        }
        ...
        public static string GetProductCategoryBreadCrumb(this ProductCategory category, IProductCategoryService categoryService, IDictionary<int, ProductCategory> mappedCategories = null)
        {
            string result = string.Empty;
