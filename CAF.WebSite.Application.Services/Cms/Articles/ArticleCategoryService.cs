...
namespace CAF.WebSite.Application.Services.Articles
{
    ...
    public partial class ArticleCategoryService : IArticleCategoryService
    {
        ...
        private const string CATEGORIES_BY_PARENT_CATEGORY_ID_KEY = "cms.articlecategory.byparent-{0}-{1}-{2}-{3}";
        private const string CATEGORIES_BY_CHANNEL_CATEGORY_ID_KEY = "cms.articlecategory.bychannel-{0}-{1}-{2}-{3}";
        ...
        public ArticleCategoryService(IRequestCache requestCache,
        ...
        public IList<ArticleCategory> GetAllProductCategoriesByParentCategoryId(int parentArticleCategoryId, bool showHidden = false)
        {
        ...
        public IList<ArticleCategory> GetAllArticleCategoriesByChannelId(int channelId, bool showHidden = false)
        {
            string key = string.Format(CATEGORIES_BY_CHANNEL_CATEGORY_ID_KEY, channelId, showHidden, _workContext.CurrentUser.Id, _siteContext.CurrentSite.Id);
            return _requestCache.Get(key, () =>
            {
                var query = _categoryRepository.Table;
                if (!showHidden)
        ...
        protected virtual IQueryable<ArticleCategory> ApplyHiddenCategoriesFilter(IQueryable<ArticleCategory> query, bool applyNavigationFilters)
        {
            ...
            if (!QuerySettings.IgnoreAcl)
            {
                query = from c in query
                        ...
                        on new { c1 = c.Id, c2 = "ArticleCategory" } equals new { c1 = acl.EntityId, c2 = acl.EntityName } into c_acl
            ...
            if (!QuerySettings.IgnoreMultiSite)
            {
                ...
                query = from c in query
                        ...
                        on new { c1 = c.Id, c2 = "ArticleCategory" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into c_sm
