        ...
        public virtual bool IsAddressValid(Address address)
        {
            ...
            if (_addressSettings.CountryEnabled)
            {
                ...
                if (_addressSettings.StateProvinceEnabled)
                {
                    ...
                    {
                        var citys = _cityService.GetCitysByProvinceId(country.Id);
                        if (citys.Count > 0)
                        {
                            if (address.CityId == null || address.CityId.Value == 0)
                                return false;

                            var city = _cityService.GetCityById(address.CityId.Value);
                            if (city == null)
                                return false;
                        }
                        if (_addressSettings.DistrictEnabled)
                        {
                            var dsstricts = _districtService.GetDistrictsByCityId(country.Id);
                            if (dsstricts.Count > 0)
                            {
                                if (address.DistrictId == null || address.DistrictId.Value == 0)
                                    return false;

                                var dstrict = _districtService.GetDistrictById(address.DistrictId.Value);
                                if (dstrict == null)
                                    return false;
                            }
                        }
